...
## 📌 نسخه ۸ — بهینه‌سازی کنترل‌های موبایل
**هدف:** تجربه لمسی حرفه‌ای‌تر — چیدمان دکمه‌ها، نمایش تمام‌صفحه، و پیام چرخش بهتر

### تغییر ۱ — Portrait Warning بهبودیافته:

| جزئیات | قبل | بعد |
|---|---|---|
| نمایش | متن ساده | آیکون 📱 با انیمیشن CSS |
| انیمیشن | هیچ | `@keyframes` — گوشی ۹۰ درجه می‌چرخه |
| عنوان | نداشت | **ROTATE YOUR DEVICE** با فونت بزرگ |
| نوار رنگی | نداشت | gradient bar زیر عنوان |
| زیرنویس | نداشت | راهنمای تکمیلی برای کاربر |

- انیمیشن چرخش با `transform: rotate()` و `@keyframes rotatePhone` ساخته شده
- ظاهر کارتی با پس‌زمینه تیره و border گرد — نه فقط یه div ساده

---

### تغییر ۲ — چیدمان دکمه‌های لمسی (سبک Dead Cells / Soul Knight):
...
[◀]                  [▶]       [🔫]
```
- دکمه Jump وسط صفحه — دسترسی سخت با شست
- چیدمان پراکنده و غیر ارگونومیک

#### بعد:
```
گوشه پایین چپ          گوشه پایین راست
┌─────────────┐        ┌──────────────────┐
│  [◀]   [▶]  │        │   [⬆ Jump]       │
│  حرکت       │        │         [🔫 Shoot]│
└─────────────┘        └──────────────────┘
...
| دکمه | موقعیت | دلیل |
|---|---|---|
| ◀ ▶ حرکت | گوشه پایین چپ | شست چپ — طبیعی‌ترین حالت |
| ⬆ Jump | بالای cluster راست | شست راست — دسترسی آسان |
| 🔫 Shoot | پایین راست cluster — بزرگ‌ترین دکمه | بیشترین استفاده = بزرگ‌ترین هدف |

- دکمه Shoot بزرگ‌ترین دکمه صفحه‌ست (thumb-friendly)
- هر دو cluster به corner گوشی چسبیده‌ — دقیقاً زیر شست‌ها

---

### تغییر ۳ — Canvas تمام‌صفحه روی موبایل:

| | قبل | بعد |
|---|---|---|
| ارتفاع canvas | `height - 130px` | تمام ارتفاع صفحه |
| جای دکمه‌ها | زیر canvas (فضای جداگانه) | **روی canvas — به‌صورت overlay** |
| فضای بازی | ۱۳۰ پیکسل کمتر — بازی له می‌شد | تمام صفحه در اختیار بازی |
| `resizeCanvas()` | `availH = window.innerHeight - 130` | `availH = window.innerHeight` |

- دکمه‌ها با `position: fixed` و `z-index` بالا روی بازی float می‌کنن
- بازی دیگه کوچیک و فشرده نمی‌شه

---

### تغییر ۴ — رفع باگ `syncControlsVisibility`:

| | قبل | بعد |
|---|---|---|
| مقدار `display` | `display: flex` | `display: block` |
| مشکل | دکمه‌ها layout اشتباه داشتن | نمایش صحیح همه دکمه‌ها |

- تابع `syncControlsVisibility()` مسئول نمایش/مخفی‌کردن کنترل‌های لمسیه
- `display: flex` با ساختار جدید overlay سازگار نبود → تغییر به `display: block`
...
| فایل/بخش | تغییر |
|---|---|
| CSS `@keyframes rotatePhone` | انیمیشن چرخش آیکون گوشی |
| `#portrait-warning` styles | کارت گرافیکی با gradient و border |
| `#touch-controls` layout | دو cluster چپ/راست با `position: fixed` |
| `resizeCanvas()` | حذف `- 130` از محاسبه ارتفاع |
| `syncControlsVisibility()` | `flex` → `block` |
...
| **CSS keyframes overlay** | دکمه‌های لمسی با `position:fixed` روی canvas float می‌کنن |
